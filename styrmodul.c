...
void straight()
{
	while (fabs(sensor1r - sensor2r) > 1.5)
	{
...
void regulateright()
{
	...
	sensormeanr = ((sensor1r + sensor2r) / 2) + 14;
	...
	else
	{
		...
		else if(((sensor1r-sensor2r) < 20) && ((sensor2r-sensor1r) < 20))
		{
			...
			else
			{
				...
				PORTC = 0x01;
				PORTD = 0x20;
				rightpwm = speed + K * (23-sensormeanr + Td * (sensormeanr_old-sensormeanr)/dt);
				leftpwm = speed - K * (23-sensormeanr + Td * (sensormeanr_old-sensormeanr)/dt);
...
void firstlap()
{
	if(myposX == startpos[0] && myposY == startpos[1] && start == 0)	//Det här kommer gälla de första sekunderna roboten börjar köra också..!
	{
		onelap=1;
	}
	else
